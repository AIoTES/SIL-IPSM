...
object AlignmentConverter {
  ...
  def convertFormatToRDFXML(xmlSource: String): String = {
    ...
    //Header properties
    val attName = getAttributeValue(alignmentNode.asInstanceOf[Element], "name",
      "new alignment")
    val attVersion = getAttributeValue(alignmentNode.asInstanceOf[Element], "version",
      "1.0")
    val attCreator = getAttributeValue(alignmentNode.asInstanceOf[Element], "creator",
      "IPSM automated converter")
    val attDescription = getAttributeValue(alignmentNode.asInstanceOf[Element], "description",
