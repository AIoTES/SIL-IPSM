...
class AlignmentCell(xmlCell: Node, rdfCell: Resource, id: String, cellFormat: CellFormat, prefixMap: Map[String,String]) extends LazyLogging {
  ...
  def applyCell(model: Model): Model = {
    ...
    val copy = if (logger.underlying.isDebugEnabled) {
    ...
    try {
      ...
      case exc: Throwable =>
        logger.error("Exception in Cell application: {}", exc.getMessage)
    }
    logger.debug(s"""${
  ...
  private def computeDatatypeInfo(): Unit = {
    ...
    logger.trace("datatypeBindingIds - BEFORE: {}", datatypeBindingIds)
  ...
  private def nonFunctionDatatypeBindigs(): String = {
    logger.trace("datatypeBindingIds - AFTER: {}", datatypeBindingIds)
