...
object AlignmentDAO {
  ...
  def findAlignmentById(id: AlignmentID) : Future[Option[AlignmentData]] = {
    val q = IPSMModel.alignments.filter{ algn => {algn.name === id.name && algn.version === id.version} }
  ...
  def addAlignment(alignData: AlignmentData): Future[AlignmentData] = {
    IPSMModel.db.run(IPSMModel.alignmentInsertQuery += alignData)
  }


  def getAlignments: Future[Seq[AlignmentData]] = {
    val allAlignmentsAction: DBIO[Seq[AlignmentData]] = IPSMModel.alignments.result
    IPSMModel.db.run(allAlignmentsAction)
  }
